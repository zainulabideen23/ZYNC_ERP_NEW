...
import { Truck, Plus, Search, X, Edit, FileText, Phone, MapPin, User, ArrowUpRight } from 'lucide-react'
...
function Suppliers() {
    ...
    const aggregates = {
        ...
        totalBalance: suppliers.reduce((sum, s) => sum + Number(s.current_balance || 0), 0)
    }
                    ...
                    <tbody>
                        {loading && suppliers.length === 0 ? (
                            ...
                            <tr key={s.id} style={{ borderBottom: index < suppliers.length - 1 ? '1px solid var(--border-light)' : 'none', background: 'var(--color-panel)', transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--color-panel-2)'} onMouseLeave={e => e.currentTarget.style.background = 'var(--color-panel)'}>
                                ...
                                <td style={{ padding: '14px 16px', textAlign: 'right', fontSize: '13px', fontWeight: 600, color: Number(s.current_balance) > 0 ? '#ef4444' : 'var(--color-text)' }}>
