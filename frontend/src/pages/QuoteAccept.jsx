...
function QuoteAccept() {
    ...
    const blockedReason = useMemo(() => {
        if (!payload) return ''
        if (payload.token_expired) return 'This response link has expired. Please request a new quotation email.'
    ...
    const handleConfirmAccept = async () => {
        ...
        try {
            ...
            navigate(`/quote/confirm/${encodeURIComponent(token)}?response=accept`, {
                replace: true,
                state: {
            ...
            <div style={{ maxWidth: '760px', margin: '0 auto' }}>
                ...
                <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '22px' }}>
                    ...
                    {!loading && !error && quotation && (
                        <>
                            <h2 style={{ margin: '0 0 8px 0', color: '#0f172a' }}>Accept Quotation</h2>
                            <p style={{ margin: '0 0 16px 0', color: '#475569' }}>
                                Please review the quotation details and confirm acceptance.
                            </p>
                            ...
                            <div style={{ marginBottom: '16px' }}>
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    placeholder="Any confirmation notes for the supplier"
                                    style={{
                                        width: '100%',
                                        borderRadius: '10px',
                            ...
                            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                                ...
                                <Link
                                    to={`/quote/reject/${encodeURIComponent(token)}`}
                                    style={{
                                        borderRadius: '10px',
                                        border: '1px solid #cbd5e1',
                                        color: '#475569',
